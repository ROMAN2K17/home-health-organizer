...
for i, p in patients.iterrows():
    ...
    with cols[i % 3]:
        ...
        if st.button(
            f"Open {p['first_name']} {p['last_name']}",
            key=f"patient_{p['id']}"
        ):
...
if selected_id:
    ...
    if not patient_rows.empty:
        ...
        for _, n in notes.iterrows():
            ...
            col1.write(
                f"**{n['created_at']}** — {n['note']}"
            )

            if col2.button(
                "🗑️",
                key=f"delete_note_{n['id']}"
            ):
                c.execute(
        ...
        new_note = st.text_area(
            "Add a new note",
            key=f"note_box_{selected_id}"
        )

        if st.button(
            "➕ Add Note",
            key=f"add_note_{selected_id}"
        ):
